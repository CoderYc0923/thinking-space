...
| 字段 | 值 | 含义 | 严重程度 |
|------|-----|------|:---:|
| **type** | 按优先级排 | 访问类型（生产最低目标，尽量达到range/ref，杜绝ALL） |          |
| | `system` | 系统表，几乎见不到 |  |
|  | `const`                 | 主键/唯一索引等值查找，最多1行，最优                         |          |
| | `eq_ref` | 关联查询时使用唯一索引查找 |  |
| | `ref` | 非唯一索引（普通索引）等值查找 |  |
| | `range` | 索引范围扫描（BETWEEN、>、<、IN 等） |  |
| | `index` | 全索引扫描，扫描整个索引树，仍有优化空间 |  |
| | `ALL` | 全表扫描，必须优化 |  |
| **rows** | 数字 | 优化器预估的扫描行数，越小越好 | — |
| **Extra      |                         | 额外信息 |          |
|  | `Using filesort` | 文件排序，没有利用索引排序，ORDER BY 字段需优化（原因：ORDER BY字段不在索引中；违背最左前缀） |  |
...
| **key_len** | 字节数 | 索引使用的字节数，可判断联合索引用了几个列 | — |
